...
class QueriesTest {
    ...
    fun `Concat with operator +`() {
        ...
        val actual = first + other // first.asSequence().concat(other)
    ...
    fun `Concat with custom eager operator`() {
        val first = listOf("Portugal", "Football", "Teams")
        val other = listOf("SLB", "The", "Champion")
        val actual = first.concat(other) // first.asSequence().concat(other)
        val expected = listOf("Portugal", "Football", "Teams", "SLB", "The", "Champion")
        assertEquals(expected, actual)
    }
    @Test
    fun `Concat with custom Lazy concat`() {
        ...
        val actual = first.concatLazy(other) // first.asSequence().concat(other)
        val expected = listOf("Portugal", "Football", "Teams", "SLB", "The", "Champion")
    ...
    fun `Collapse with custom Lazy collapse using yield`() {
        val actual: Sequence<Int?> = sequenceOf(null, 4, 5, 5, 6, 7, 7, 5, 3, 3, 7, 6, 5)
            .collapse()
        val expected = listOf(null, 4, 5, 6, 7, 5, 3, 7, 6, 5)
        assertEquals(expected, actual.toList())
    }
    @Test
    fun `Interleave with custom Lazy interleave using yield`() {
        ...
        val actual = first.interleave(other) // first.asSequence().concat(other)
